...
public class UnitTest1
{
    ...
    {
        public static string GetForNumber(int number)
        {
            if (number % 15 == 0)
            {
                return "FizzBuzz";
    ...
    /// <summary>
    /// Just Verifies the first 100 from a hard coded list... can see the patterns the way FizzBuzzValues are laid out.
    /// </summary>
    public class FixBuzz
    {
        private static readonly Dictionary<int, string> FizzBuzzValues = new()
        {
        ...
        public string[] Process()
        {
            ...
            for (var i = 0; i < 100; i++)
            {
                // Uses implementation to generate the numbers, but verifies first 100 against a list.
        ...
        {
            Debug.Assert(output.Length <= 100);
            
            for (var i = 0; i < output.Length; i++)
            {
                var key = i + 1;
                if (!FizzBuzzValues.TryGetValue(key, out var value))
                {
                    return false;
                }
